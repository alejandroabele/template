...
const formSchema = z.object({
  ...
  fechaFabricacionEstimada: z.string({ message: "Requerido" }),
...
export default function PresupuestoAlmacenForm({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      id: data?.id,
      fechaFabricacionEstimada: data?.fechaFabricacionEstimada || today(),
      observaciones: "",
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="bg-muted/30 p-4 rounded-lg">
            <DatePicker
              label="Fecha de Fabricacion comprometida"
              form={form}
              name="fechaFabricacionEstimada"
              fromYear={2025}
              className="w-full"
            />
            <p className="text-xs text-muted-foreground mt-2 flex items-center">
              ...
              comprometerse a finalizar el trabajo
            </p>
          </div>
          <div className="p-4 ">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    ...
                    modifico la fecha de fabricación
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
