...
export const Ingresos: React.FC<IngresosProps> = ({
  ...
  // 4. Extraer y agrupar datos de ingresos por rubro
  ...
  // 5. Renderizar
  const isSeccionCollapsed = !expanded.secciones.has("ingreso");
  ...
  return (
    <>
      {/* Header de Ingresos */}
      <TableRow
        className="bg-emerald-200 border-t border-emerald-300 font-bold cursor-pointer hover:bg-emerald-200"
        onClick={() => toggleSeccion("ingreso")}
      >
        <TableCell
          className="py-2 px-3 text-emerald-900 text-sm w-[200px] sticky left-0 z-10"
        ...
        {columnas.map((fecha) => {
          const valor = totalesPorDia[fecha]?.ingresos || 0;

          return (
            <TableCell key={fecha} className="py-2 px-3 text-center">
              <div className="bg-white rounded p-1.5 shadow-sm">
                <div className="font-bold text-sm whitespace-nowrap text-emerald-700">
                  {formatMoney(valor)}
                </div>
              </div>
            </TableCell>
        ...
        <>
          {rubros.length > 0 ? (
            ...
            <TableRow>
              ...
              >
                No hay categorías de ingresos configuradas
              </TableCell>
            </TableRow>
          )}
        </>
