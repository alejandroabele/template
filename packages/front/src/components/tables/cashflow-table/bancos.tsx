...
export const Bancos: React.FC = () => {
  ...
  const saldosPorColumna = useMemo(() => {
    ...
    if (vista === "semanal-mes" && columnasData.semanasInfo) {
      // Para semanal-mes, mapear cada semana a su fecha fin (último día)
      const semanasInfo = columnasData.semanasInfo;
      columnasData.fechas.forEach((semana) => {
        const info = semanasInfo[semana];
  ...
  const bancos = (saldosPorColumna[primeraColumna]?.saldos ||
    []) as SaldoBancoAPI[];
  const isCollapsed = !expanded.secciones.has("bancos");

  // Calcular totales por columna
  const totalesPorColumna: Record<string, number> = {};
  Object.keys(saldosPorColumna).forEach((columna) => {
    totalesPorColumna[columna] = saldosPorColumna[columna]?.total || 0;
  });
  ...
  return (
    <>
      {/* Header de Bancos */}
      <TableRow
        className="bg-slate-100 border-t border-slate-200 font-bold cursor-pointer hover:bg-slate-100"
        onClick={() => toggleSeccion("bancos")}
      >
        <TableCell
          className="py-2 px-3 text-slate-700 text-sm w-[200px] sticky left-0 z-10"
        ...
        {columnas.map((fecha) => {
          const valor = totalesPorColumna[fecha] || 0;
          const valueColor = valor >= 0 ? "text-slate-700" : "text-rose-700";
          ...
          return (
            ...
            >
              ...
              {columnas.map((fecha) => {
                ...
                const monto = saldoBanco ? Number(saldoBanco.monto) : 0;
